...
class BaseAgent(ABC):
    ...
    def _build_macro_header(inputs: dict[str, Any]) -> str:
        ...
        lines: list[str] = []
        ea = (
            inputs.get("economy_analysis")
            or inputs.get("economy_analysis_fi")
            or inputs.get("economy_analysis_pm")
        )
        ms = (
            inputs.get("macro_scenario")
            or inputs.get("macro_scenario_fi")
            or inputs.get("macro_scenario_pm")
        )
