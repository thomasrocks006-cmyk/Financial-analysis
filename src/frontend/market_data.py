...
async def _fetch_fmp(ticker: str, client: httpx.AsyncClient, api_key: str) -> dict[str, Any]:
    ...
    try:
        data = await _fmp_get(
            client, "/income-statement", api_key, {"symbol": ticker, "limit": "1"}
        )
    ...
    try:
        data = await _fmp_get(
            client, "/cash-flow-statement", api_key, {"symbol": ticker, "limit": "1"}
        )
...
async def _fetch_finnhub(ticker: str, client: httpx.AsyncClient, api_key: str) -> dict[str, Any]:
    ...
    try:
        data = await _finnhub_get(
            client, "/stock/metric", api_key, {"symbol": ticker, "metric": "all"}
        )
...
def _cross_validate(fmp: dict, finnhub: dict) -> list[dict]:
    ...
    comparisons = [
        ...
        (
            "52w_high",
            fmp.get("fmp_quote", {}).get("52w_high"),
...
async def _fetch_ticker_all(
    ...
    has_price = fmp_data.get("fmp_quote", {}).get("price") or finnhub_data.get(
        "finnhub_quote", {}
    ).get("price")
    if not has_price:
...
def _build_unified_snapshot(ticker: str, raw: dict[str, Any]) -> dict[str, Any]:
    ...
    snap: dict[str, Any] = {
        "ticker": ticker,
        "company_name": fp.get("company_name")
        or fq.get("company_name")
        or yq.get("company_name")
        or ticker,
        ...
        "market_cap_bn": fq.get("market_cap_bn")
        or fkm.get("market_cap_bn")
        or yq.get("market_cap_bn"),
        ...
        "gross_margin_pct": fi.get("gross_margin_pct")
        or (hm.get("gross_margin_ttm") and round(hm["gross_margin_ttm"] * 100, 1)),
        ...
        # Balance sheet / risk
        "debt_to_equity": fkm.get("debt_to_equity")
        or hm.get("debt_equity")
        or yq.get("debt_to_equity"),
...
async def fetch_universe(
    ...
    async with httpx.AsyncClient() as client:
        tasks = [_fetch_ticker_all(ticker, client, fmp_key, finnhub_key) for ticker in tickers]
