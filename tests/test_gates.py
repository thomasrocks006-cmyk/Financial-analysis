...
from research_pipeline.schemas.claims import ClaimLedger, Claim, ClaimStatus, EvidenceClass, ConfidenceLevel
...
class TestGate0Configuration:
    def test_all_valid(self):
        result = PipelineGates.gate_0_configuration(
            api_keys_valid=True, config_loaded=True, schemas_valid=True,
        )
    ...
    def test_missing_api_keys(self):
        result = PipelineGates.gate_0_configuration(
            api_keys_valid=False, config_loaded=True, schemas_valid=True,
        )
    ...
    def test_missing_config(self):
        result = PipelineGates.gate_0_configuration(
            api_keys_valid=True, config_loaded=False, schemas_valid=True,
        )
        assert result.passed is False

    def test_all_missing(self):
        result = PipelineGates.gate_0_configuration(
            api_keys_valid=False, config_loaded=False, schemas_valid=False,
        )
        assert result.passed is False
...
class TestGate2Ingestion:
    def test_all_ingested(self):
        ...
        result = PipelineGates.gate_2_ingestion(
            ingest_results=ingest_results, universe=["NVDA", "AVGO"],
        )
        assert result.passed is True
...
class TestGate3Reconciliation:
        ...
        report = ReconciliationReport(
            ...
            fields=[
                ReconciliationField(
                    ...
                    source_a="fmp", source_a_value=125.0,
                    source_b="finnhub", source_b_value=125.3,
    ...
    def test_with_reds(self):
        report = ReconciliationReport(
            ...
            fields=[
                ReconciliationField(
                    field_name="target_median", ticker="NVDA",
...
class TestGate5Evidence:
    def test_valid_ledger(self):
        ledger = ClaimLedger(
            ...
            claims=[
                Claim(
                    claim_id="CLM-001", run_id="test", ticker="NVDA",
                    claim_text="Revenue grew", evidence_class=EvidenceClass.PRIMARY_FACT,
                    source_id="SRC-001", status=ClaimStatus.PASS,
                ),
            ],
        )
    ...
    def test_fail_claims_block(self):
        ledger = ClaimLedger(
            ...
            claims=[
                Claim(
                    claim_id="CLM-001", run_id="test", ticker="NVDA",
                    claim_text="Bad claim", evidence_class=EvidenceClass.PRIMARY_FACT,
                    source_id="SRC-001", status=ClaimStatus.FAIL,
                ),
            ],
        )
        result = PipelineGates.gate_5_evidence(ledger=ledger)
...
class TestGate11Review:
    def test_review_pass(self):
        review = AssociateReviewResult(
            run_id="test", status=PublicationStatus.PASS, issues=[],
        )
...
class TestGate12Portfolio:
    def test_valid_portfolio(self):
        result = PipelineGates.gate_12_portfolio(
            variants_count=3, constraint_violations=None, review_passed=True,
        )
        assert result.passed is True

    def test_too_few_variants(self):
        result = PipelineGates.gate_12_portfolio(
            variants_count=2, constraint_violations=None, review_passed=True,
        )
        assert result.passed is False
